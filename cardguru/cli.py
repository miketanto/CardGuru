...
def cmd_puzzle_gen(args):
    from .cardstore import CardStore
    from .puzzlegen import (generate, generate_t2, generate_t3,
                            generate_t4, write_puzzles)

    store = CardStore.open(args.db)
    if store is None:
        sys.exit(f"card store not found at {args.db} "
                 "(build it with: python -m cardguru cardstore)")
    gen = {1: generate, 2: generate_t2, 3: generate_t3,
           4: generate_t4}[args.tier]
...
def main(argv=None):
    ...
    pg.add_argument("--tier", type=int, choices=[1, 2, 3, 4], default=1)
