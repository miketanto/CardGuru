...
set -u
# sweep2: waits for the first sweep (a, b) to finish, then runs c and d
until grep -q "SWEEP COMPLETE" /home/user/CardGuru/rl/artifacts/cardemb_sweep/RESULTS.md 2>/dev/null; do sleep 120; done
...
declare -A CFG
# (c) = (a) + weighted-BCE readout reconstruction (rare mechanical bits under-fit by MSE: a failed ans_minus_toughness F1 0.725)
CFG[c]="--struct tree --tree --piece-dropout 0.2 --readout-loss bce $RECIPE"
# (d) = (b) + the same
CFG[d]="--struct script --readout-loss bce $RECIPE"
...
if [ $# -gt 0 ]; then
  ...
  for n in c d; do run_one "$n" "${CFG[$n]}"; done
fi
