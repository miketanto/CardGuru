...
Five rungs were pre-registered; A0, A1, A2, A3 and A4L all ran. Two findings emerged that were not on the runbook and matter more than most of what was.

**A0 — can it fit? PARTIAL, and the fail branch is excluded.** Training top-1 0.9795 against a 0.99 bar, but the slice's exact-top-1 copy ceiling is 0.9810, so the bar was above what the metric could reach; 0.9984 of ceiling, class top-1 0.9960, value EV 0.9908 (bar 0.95, met). Card-path gradients, masking and fit-reachability all checked clean, so "the plumbing is broken" is positively excluded.

**A1 — DATA-LIMITED, carried entirely by the policy half.** Last doubling: +0.0118 top-1 (bar 0.01) and -0.0019 EV (bar 0.03). The halves disagree and are reported separately: more data buys imitation accuracy and does not buy outcome prediction. Both halves reproduce their parents (13b to <=0.004 top-1; D1's N1 EV 0.095, best epoch 2, val_mse 0.5600). A1 gives NO evidence that more data fixes the critic, which qualifies D1's "data-limited" inference.

**A2 — probe 1 reads DISCARDS by the bar as written, and A3 was triggered.** Token 0.8590 off-wire against a 0.8727 ceiling (0.984, clause 1 met) but only +0.0766 above the card-blind floor (bar 0.25, inside the within-0.10 fail branch). The probe as pre-registered is SATURATED - the card-blind net scores 0.9792 on it - which is why an off-wire card-identity split was added beside it. Probe 2 contradicts the floor clause: embedding-row-only swaps move bc.pt 3.2x the card-blind net, disjoint intervals. **Central finding: the card-blind clone reaches priority top-1 0.848 against bc.pt's 0.876 - deleting card identity costs 0.028 and nothing on blocks - so on this deck the network leans mostly on wire fields, not card identity.**

**A3 — the remedy works by the bar (both clauses met), with the caveat cutting both ways.** Agreement cost -0.0007; probe recovery 0.943 of ceiling. But bc.pt already sat at 0.984 of that ceiling, so clause 2 was satisfied pre-remedy; on the clause A2 actually failed the remedy halved the floor gap on the narrow population and exceeded BC on the pooled one. Probe 2 says the remedy DID move weight from wire fields to card identity (embedding-row dp 1.7x, disjoint) while strict flips fell - two behavioural measures disagreeing in sign, stated as such.

**A4L — first series measured a training pathology; second series answers the clauses.** Four rungs recorded with six of seven clones frozen at epoch 1, arms identical to four decimals, and the shared policy EQUAL to a fixed-position rule. Rebuilt at --logit-bound 0: aspect-is-learnable MET on both re-run rungs (0.910, 0.901 vs first-series 0.734, 0.766), shared-transfers MET with content, aspect-does-not-transfer NOT met, joint neither helps nor hurts. **The shared game transfers and the aspect does not fail to transfer - but with joint attack/block uncloned this means only that the new card's presence does not disturb priority and target choice.**

**Cross-cutting finding 1 - CombatMath coverage bounds every combat result here.** On four W-decks only 0.56-0.65 of CP7's declared attack/block sets are in the candidate list our own policy selects from (BenchDimir: 0.948/0.974), and coverage is opponent-dependent (lane H 0.833/0.867 vs CP7 mirror 0.503/0.450). This bounds expressible combat play independent of network, teacher and signal.

**Cross-cutting finding 2 - the logit bound froze training in one regime.** B = 5 with logits = B*tanh(raw/B): frozen clones sit at mean abs raw logit 54-110 (gradient ~1e-9 to ~1e-19), trained ones at 17-21. Relaxing it is worth +0.09 to +0.14 held-out top-1 on the same data, and it rescues frozen runs without inflating healthy ones. It does NOT reach the published Phase 13 rows: bc.pt measures 7.57 and the D2b RL trajectory drifts only 7.57 -> 8.09 over 1,024 episodes, so the saturation explanation I proposed for 13c's small per-update KL is WITHDRAWN.

**What the phase cannot settle.** Nothing here is a win rate or says the agent plays better. One seed almost everywhere. A4L covered four of eleven white rungs and none of the black branch. The Phase 14 verdict (the win/loss signal from this encoding is too weak for this RL at this scale) is untouched.
