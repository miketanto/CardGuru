...
- 2026-09-13 19:30 (WSL 18:07): 8a-b 256 DEVELOPMENT PROBE (50 games): D0 17/50 = 0.34 [0.224,0.478], attacks 305/325, blocks 4/98, 19.5 turns; sampled jobs 22/20/19/25 of 64 (vs 8a 3/0/0/0), ~40 consults per game, 0.55-0.84 games/s. Batch centring changes the first job already (same init, same game seeds: the first update after 32 episodes). Census ck_256 read next; 512 level expected ~18:35 WSL.
- 2026-09-13 19:55 (WSL 18:25): 8a-b DONE (rc=0, 25 min): 512 LEVEL D0 41/100 = 0.41 [0.319,0.508], D1 0.29 [0.210,0.385]; 256 dev probe 17/50; ck_512 census argmax-SPELL 228/257, P(SPELL) 0.88 at >=3 lands, argmax-land 1.00, ATTACK 111/111, BLOCK 1/47; sampled curve flat .33 -> .34 (first-4 / last-4); KL-stopped 4/16. Row "8a-b - C1 recipe + --adv-norm batch on Dimir" written: ESCAPES THE RAIL yes, TRAINS yes on the 8a-b bar (clear above 0.037), NOT on the original 8a bar (0.5 inside the interval, flat curve); B2 third-land cost absent at 512; 8a-c not run. 8B LAUNCHED 18:24 WSL per the pre-stated consequence (rl/run_8b.sh: L2 from L0 ck_3072, Dimir entry = 8ab/s0/ck_512.pt, R0_EVERY=256, D0 50-game probes per block / 100 at 4096, then 50-game 4-row suites on L2 ck_4096 and L0 ck_3072, then H2H 100; log rl/artifacts/v7/8b/run_8b.log, lane /tmp/rl_8b_L2, driver 7912, ports 7952/7960); verified by pgrep; idle 7910 stopped. Expect ~4 x (256 eps + 50-game probe) ~ 1.5-2 h + suites ~30 min + H2H.
- 2026-09-13 20:00 (WSL 18:26): KEEPALIVE made independent of the agent session: PowerShell `Start-Process cmd -ArgumentList '/c start "keepalive" /min wsl.exe -e bash -lc "sleep 18000"' -WindowStyle Hidden` -> a wsl.exe pair (PIDs 4764/27264, started 18:25 local, 5 h) that survives this session ending (a plain Start-Process wsl.exe -WindowStyle Hidden exited at once). The Bash-tool keepalives (bgvl4f9qj to ~19:40, bd74u6xgx to ~21:50) are redundant now. 8b running: first block (heuristic:BenchDimir, 256 eps) + 50-game probe expected ~19:05 WSL; rows in /tmp/rl_8b_L2/lane.log (R0| and R0_OPP| lines), jobs in jobs.log; the runner post-processes to rl/artifacts/v7/8b/L2 and runs the suites + H2H itself. NEXT when 8B|done: read L2 rows (3072 +0 / 3328 / 3584 / 3840 / 4096), suite_L2_4096.txt, suite_L0_3072.txt (pooled 200 games each), h2h.txt; write the 8b row against the pre-registration + Amendment 2; then B3 (8c: not run - reason: 8a as-is failed, 8a-b is one seed at 512; write it), B4 (handoff UPDATE + section M).
- 2026-09-13 20:40 (WSL 19:30): 8B DONE (rc=0; lane 28 min, suites + H2H 19 min). Q4 ROW WRITTEN ("8b / Q4 - L2 ... vs L0"): pooled 4-row suites IDENTICAL 153/200 = 0.765 [0.702,0.818] each (L2 -4 W0Base, -3 Dimir, +6 Burn, CP7 +1 with 12 -> 0 stalls); home 4096 D0 0.73 [0.636,0.807] clear below L0 +1024 0.91 (regression); H2H L2 0.32 [0.237,0.417], 20 stalls; per-block sampled 0.70 / 0.25 (CP7 Burn) / 0.73 (8a-b Dimir) / 0.62, 1 stall in 1,024; mechanism = racing (23.9 vs 32.4 turns, under/over 118/92 vs 163/162), not L1 stalling. Verdict: no difference shown on the suite; hurts at home and head-to-head. 8c NOT RUN (section "8c - not run"). Nothing running in WSL (drivers 7910-7914 stopped, no servers); Bash keepalives stopped; the hidden wsl.exe keepalive stopped via PowerShell. B4 next: HANDOFF UPDATE + section M.
- 2026-09-13 20:50 (WSL 19:40): B4 DONE - HANDOFF-V7.md UPDATE line + section M (Q3/Q4 one-liners, owed items 1-7). PHASE 8 CLOSED. Nothing running; keepalives stopped.
