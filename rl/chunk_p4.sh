...
# per P4D1-SPEC.md; evals are argmax on a named deck.
...
if [ "$MODE" = "train" ]; then
    ...
    mvn -q -pl Mage.Tests surefire:test -Dtest='RLEpisodeDriver' \
        ...
        -Drl.opponent=search -Drl.searchPlies=1 -Drl.searchBreadth=8 -Drl.policy=socket -Drl.port=$PORT \
        -Drl.deck=$POOL -Drl.stopTurn=80 $JFEAT \
        -Drl.mode=train -Drl.seed=$((9000000 + ARMOFF + SEED*1000000 + trained)) \
    ...
    echo "P4D1|arm=$ARM|seed=$SEED|trained=$trained|train_chunk_done"
else
    ...
    echo "P4D1|arm=$ARM|seed=$SEED|trained=$trained|eval=$TAG|deck=$DECK|win_rate=$wr|stalls=$stalls" \
        | tee -a $OUT/curve.txt
fi
kill $SERVER 2>/dev/null
