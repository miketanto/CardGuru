#!/bin/bash
# Phase 12 Amendment 4: the SAMPLED-play counterpart of rl/battery_xdeck.sh (a copy with the play
...
for row in $ROWS; do
    ...
    if [ -n "$w" ] && [ -n "$e" ]; then
        echo "XDECKS|ck=$(basename $CK)|agent=$ADECK|vs=$KIND|oppDeck=$DK|$(wil $w $e)|$(grep -o 'losses=[0-9]*\|draws=[0-9]*\|stalls=[0-9]*\|turns_per_ep=[0-9.]*' $f | paste -sd' ')"
    else
        echo "XDECKS|ck=$(basename $CK)|agent=$ADECK|vs=$KIND|oppDeck=$DK|NA|$(grep -h 'RLJOB|error\|Exception' $OUT/driver_${KIND}_${DK}.log 2>/dev/null | head -1 | cut -c1-120)"
    fi
    i=$((i + 1))
done
