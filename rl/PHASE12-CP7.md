...
- 2026-09-15 10:59Z WSL: AMENDMENT 5 FLAG IMPLEMENTED - rl/policy_server.py --argmax-two-stage (_argmax_two_stage: act iff sum P(non-PASS) > P(PASS), then _argmax_classes over the acting candidates with PASS masked; fallback to _argmax_classes without both kinds; eval branch only, default OFF = old readout). Developed as policy_server.py.new, tested in a symlinked copy (19/20; the one failure test_deck_ctx_artifact_loads_and_is_cached fails identically with the ORIGINAL file in that copy - ModuleNotFoundError 'cardguru' from the copy's package layout, not the patch), swapped atomically (mv) at 10:58:37Z while the sampled battery's 7949 server had been up 288 s (its code already loaded; default off cannot change its behaviour). Gates on the real tree after the swap: tests/test_v7_server.py 20 passed (2 new: test_argmax_two_stage_readout, test_act_v7_eval_honours_argmax_two_stage), rl/v7_check.py checks=15 failures=0 (100 s).
- 2026-09-15 11:01Z WSL: TWO-STAGE RUNNER QUEUED - rl/run_p12t.sh detached (pgrep-verified), waiting for the sampled battery's P12S|done and a free port 7949; then the two-stage levels (rl/battery_xdeck.sh unchanged, SRVEXTRA '--device cuda --logit-bound 5 --argmax-classes --argmax-two-stage', eval mode, level seeds, 7949/7914) on the six snapshots (start, 7,424, 8,448, 9,472, 10,496, 11,520; CP7 100 + heuristic 50 each), then the CPU readouts on the censuses n=8 (owed), n=11, n=15, n=19, n=20. Log rl/artifacts/v7/12/twostage/p12t.log (lines P12T|<point>|XDECK|..., P12T|READOUT|..., P12T|done). Estimated: sampled done ~13:15Z, two-stage ~15:30Z, readouts ~15:45Z; a fresh 18 h keepalive covers it.
- 2026-09-15 ~12:50Z WSL: EVALUATIONS STOPPED BY THE USER (~12:40Z, rl/stop_p12_evals.sh, main session: 'enough to conclude'); nothing resident verified. Sampled battery finished 4 of 6 snapshots: CP7 sampled 0.120 [0.070,0.198] / 0.110 [0.063,0.186] / 0.150 [0.093,0.233] / 0.220 [0.150,0.311] at start / 7,424 / 8,448 / 9,472; heuristic sampled 0.720 / 0.580 / 0.640 (first three). AMENDMENT 5 CANCELLED BY THE USER, NOT RUN (flag kept, default off); the n=8/11/15/19/20 census readouts cancelled. AMENDMENT 4 READING: 'argmax artifact' - readout half met (0.153 -> 0.167 -> 0.428), win-rate half met (sampled flat-then-rising 0.12 -> 0.22 while argmax fell 0.19 -> 0.08); 'real regression' ruled out for those snapshots; the rise is modest and not clear (intervals overlap on [0.150,0.198]). Correction in the open noted under the verdict for the second/third level rows' 'falling' wording. PHASE 12 VERDICT written (V7-VALIDATION 'Phase 12 verdict'); HANDOFF-V7 UPDATE + section P. PHASE 12 CLOSED FOR TONIGHT (Dimir only; landfall/white paused, resumable).
