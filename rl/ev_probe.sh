...
EPS=${3:-128}
PORT=${4:-7991}
OUT=${EV_OUT:-/tmp/rl_ev_$(basename $CKPT .pt)_${DECK}}
...
RL_TORCH_THREADS=1 setsid nohup python3 $RL/policy_server.py --port $PORT \
    ...
    --lr 3e-4 --log "$OUT/train.csv" > "$OUT/server.log" 2>&1 &
...
RL_PERSIST=1 RL_AUTOSTART=1 RL_CONC=2 timeout 5400 bash $RL/run_driver.sh \
    ...
    -Drl.encoderV=6 -Drl.deck=$DECK.dck -Drl.oppDeck=$DECK.dck \
...
echo "update,episodes,steps,batch_wr,value_ev"
