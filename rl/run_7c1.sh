#!/bin/bash
# 7c1 (V7-VALIDATION "C1 pre-registration"): the fixed recipe to 2,048 episodes on three
# seeds. B2's optimiser (lr 3e-5, 1 epoch, logit bound 5, AdamW wd 0.01 on the heads)
# + --adv-norm auto + --target-kl 0.02 + --argmax-classes (eval only), mana-ability
...
# Launch: setsid nohup bash rl/run_7c1.sh > <artifacts>/7c1/run_7c1.log 2>&1 &
[ -f ~/.profile ] && . ~/.profile
...
mkdir -p $ART
run_seed() {   # $1 seed
    local OUT=/tmp/rl_7c1_s$1
    if [ -s $ART/s$1/census.txt ]; then echo "7C1|seed=$1|skip=done"; return; fi
    mkdir -p $OUT
    echo "7C1|seed=$1|start=$(date -u +%FT%TZ)"
    ...
    echo "7C1|seed=$1|rc=$?|end=$(date -u +%FT%TZ)"
