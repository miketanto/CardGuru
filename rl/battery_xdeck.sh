#!/bin/bash
# Phase 8 A3: the CROSS-DECK SUITE of one checkpoint, outside a lane, under the fixed
...
for row in $ROWS; do
    ...
    OPPF="-Drl.opponent=$KIND"; [ "$KIND" = cp7 ] && OPPF="$OPPF -Drl.aiSkill=6"
    if [ ! -s "$f" ]; then
    ...
    if [ -n "$w" ] && [ -n "$e" ]; then
        TW=$((TW + w)); TG=$((TG + e))
        echo "XDECK|ck=$(basename $CK)|agent=$ADECK|vs=$KIND|oppDeck=$DK|$(wil $w $e)|$(grep -o 'losses=[0-9]*\|draws=[0-9]*\|stalls=[0-9]*\|turns_per_ep=[0-9.]*\|attackBudgetHit=[0-9]*' $f | paste -sd' ')"
    else
        echo "XDECK|ck=$(basename $CK)|agent=$ADECK|vs=$KIND|oppDeck=$DK|NA|$(grep -h 'RLJOB|error\|Exception' $OUT/driver_${KIND}_${DK}.log 2>/dev/null | head -1 | cut -c1-120)"
    fi
    i=$((i + 1))
done
