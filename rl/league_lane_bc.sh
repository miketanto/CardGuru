...
OUT=/tmp/rl_league_bc_s${SEED}
# BC-init arm: seeded from the C1 v3 BC student instead of scratch -
# init is the ONLY difference vs league_lane.sh (same league, same
# no-yields/no-shaping regime, same probes)
mkdir -p $OUT
if [ ! -f $OUT/net.pt ]; then
    cp /tmp/rl_p5_c1/student_v3.pt $OUT/net.pt
fi
DECK=BenchDimir.dck
mkdir -p $OUT/pool
...
start_server() {  # $1 ckpt path, $2 port, $3 logfile -> pid in $OUT/.srvpid
    ...
    echo "LEAGUE_FAILED|bc_s$SEED|server_never_started|$3"
    cat $3
    exit 1
}
...
while true; do
    ...
    if [ "$rows_after" -le "$rows_before" ]; then
        echo "LEAGUE_FAILED|bc_s$SEED|no_training_rows (trained stays $trained)"
        exit 1
    fi
    trained=$((trained + 64))
    ...
    if [ $((trained % 256)) -eq 0 ] || [ "$trained" -ge "$BUDGET" ]; then
        for PROBE in heuristic search; do
            ...
            echo "LEAGUE|arm=bc|seed=$SEED|trained=$trained|probe=$PROBE|win_rate=$wr|stalls=$stalls" \
                | tee -a $OUT/curve.txt
        done
    fi
done
echo "LEAGUE_DONE|arm=bc|seed=$SEED|trained=$(cat $OUT/trained.txt)"
