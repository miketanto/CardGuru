...
- 2026-09-13 10:25 (WSL 04:12): BC row written ('7d piece (2) - BC'): BC battery (7d3 s0 trained=0, fixed protocol) D0 0.58 [0.482,0.672] / D1 0.59 / TWIN 0.57; L0 +0 row (C1 s0 ck_2048 under the fixed protocol) D0 0.86 [0.779,0.915] / D1 0.87 / TWIN 0.88 (plain-argmax was 0.85/0.85/0.88, paired seeds). D-PPO s0 training; L0 training; re-battery running. NEXT: rows as they land (Monitors on 7d3/lanes_R0.log, 7l/lanes_R0.log, rebattery.log); the C1 correction paragraph once rebattery s0+s1 2048 are in.
- 2026-09-13 11:10 (WSL 05:05): L0's trained=2560 battery row is NA: driver 7912 jobs 12-14 returned rc=1 in 0.0 s (no exception in the JVM log, no connection reached the battery server on 7950; pinned properties identical to the working 2048 battery; a manual eval probe on 7912 with the lane's exact flags works, rc=0) - transient, cause NOT captured (the lane sends the client's RLJOB|error to /dev/null). Recovery: battery_ck.sh on /tmp/rl_7l_L0/ck_2560.pt running now (server 7948 / driver 7914, out rl/artifacts/v7/7l/L0_rebat/); rl/fill_na_batteries.sh written for any later NA row (run after a lane ends: fill_na_batteries.sh /tmp/rl_7l_L0 rl/artifacts/v7/7l/L0_rebat). Rebattery C1 s0 2048 under the fixed protocol = 0.86/0.87/0.88 (identical to the L0 +0 row, same seeds: consistent). D-PPO s0 at its 512 battery next - check for the NA signature.
- 2026-09-13 15:15 (WSL 06:15): MEMORY STALL - between WSL 05:05 and 06:04 D-PPO s0 made 1 update and L0 6: WSL box is 11 GB, `free` showed 0 available with four policy servers (7940 D-PPO, 7950 L0, 7947 C1 re-battery, 7948 L0-2560 recovery) + three driver JVMs resident (swapping; GPU 4 %, load 12/16). Stopped ONLY the two side jobs with rl/stop_rebatteries.sh (servers 7947/7948, drivers 7913/7914; both jobs resumable) -> 3 GB available, lanes untouched. RULE for this box: at most TWO policy servers + two driver JVMs at a time; run re-batteries one at a time and only when a lane is finished. Still owed: C1 s1 ck_2048 re-battery (fixed protocol) and L0 ck_2560 recovery - rerun after D-PPO s0 or L0 ends. The stall inflates wall-clock only (update_s stayed 19-28 s); no result is affected.
- 2026-09-13 12:45 (WSL 06:40): L1 STOPPED (rl/stop_7l.sh; it had not started its lane). CAUSE of the L0 NA rows FOUND: a fork race in rung0_lane.sh start_server (the background job's '> server.log' truncation happens after the parent's first grep, which reads the OLD server's 'policy server' line, returns at once, and the probes hit a server that has not bound yet -> driver jobs rc=1 in 0.0 s; never the first battery of a lane; won by the parent twice under load). FIXED in rung0_lane_league.sh (': > server.log' before the launch, same for the opp server); rung0_lane.sh gets the same fix when D-PPO no longer runs it (edit forbidden while a lane executes the script); D-PPO's later NA rows (if any) are recovered by rl/fill_na_batteries.sh. RUNNING: rl/recover_L0.sh (ck_2560 then ck_3072, server 7948 / driver 7914, log rl/artifacts/v7/7l/L0_rebat/recover.log, rows in rows.txt). L0 row written (partial; recovered rows appended when they land). Memory: a JVM is 3.4 GB RSS, a server 1.7 GB -> L1 (server + opp server + JVM) waits for D-PPO to END; then smoke (4-game) + relaunch run_7l.sh (L0 skipped, census.txt present). C1 s1 ck_2048 re-battery also after D-PPO.
- 2026-09-13 12:55 (WSL 06:50): scheduling change (coordinator): D-PPO PARENT stopped (rl/stop_7d3_parent.sh; seed-0 lane on 7910/7940 untouched, at its 1024 battery; seed 1 does not auto-start). rl/wait_7d3_s0.sh detached (log rl/artifacts/v7/7d3/wait_s0.log): at R0_DONE|s0 it runs post_lane_seed.sh -> 7d3/s0 and fill_na_batteries.sh (7949/7915) for any NA row. THEN, in order: D-PPO seed-0 section (one-seed Q1 vs C1's two seeds); the server.log truncation fix into rung0_lane.sh; 4-game league smoke + relaunch rl/run_7l.sh (L0 skipped); C1 s1 ck_2048 re-battery while L1 trains; Q2 row when L1 ends; then relaunch rl/run_7d3.sh for seed 1.
- 2026-09-13 13:25 (WSL 07:18): L0 recovered rows written (+512 D0 0.92 / +1024 D0 0.91, fixed protocol). Q2 PRE-REGISTRATION AMENDED before L1 trains: heuristic-family batteries cannot decide (ceiling); deciding = (a) head-to-head L1 vs L0 at ck_3072 and (b) each arm vs CP7, 100 games, fixed protocol (rl/hard_battery.sh). RUNNING: hard_battery.sh for L0 ck_3072 vs CP7 (server 7948 / driver 7914; log rl/artifacts/v7/7l/hard/L0_3072.log). D-PPO s0 lane still training (its waiter is armed).
- 2026-09-13 13:55 (WSL 07:50): D-PPO seed 0 DONE and written ('7d D-PPO - seed 0'): 2048 D0 0.68 [0.583,0.763] clear BELOW C1 pooled 0.850 -> one-seed Q1 = 'IL hurts'; dead stretch gone (163/256), sampled flat ~0.70, argmax-PASS 0 %, attack/block everything, 22-turn games; 1024 dip (stopped attacking, 47 turns, 0.50). rung0_lane.sh got the server.log truncation fix (no lane running). League smoke (rl/smoke_league.sh, 4-game batteries, one 64-episode league chunk) running -> then relaunch run_7l.sh (L0 skipped), then the C1 s1 ck_2048 re-battery beside L1.
- 2026-09-13 14:05 (WSL 08:00): league smoke PASSED (rl/artifacts/v7/7l/smoke_league.log: +0 battery 4 games, one 64-episode league chunk with 4 opp-server conns, battery after training 4 games, rc=0). CHAIN launched (rl/chain_s1rebat_then_7l.sh, log rl/artifacts/v7/7l/chain.log): C1 s1 ck_2048 re-battery ALONE first (memory: L1 = JVM + 2 servers), then rl/run_7l.sh (L0 skipped) -> L1 on driver 7912, ports 7951/7960, state /tmp/rl_7l_L1, rows mirrored to rl/artifacts/v7/7l/lanes_R0_L1.log, run log rl/artifacts/v7/7l/run_7l.log. After L1: hard_battery.sh for L1 ck_3072 (vs CP7 + head-to-head vs L0 ck_3072), Q2 row, then rl/run_7d3.sh (seed 1), then the remaining C1 re-batteries (rl/rebattery_c1.sh, resumable).
- 2026-09-13 14:20 (WSL 08:15): C1 s1 ck_2048 re-battery DONE (fixed protocol D0 0.86 / D1 0.83 / TWIN 0.89; plain 0.85/0.82/0.88): correction paragraph written - C1 two-seed level 0.860 [0.805,0.901] under the fixed protocol, the fix moves nothing at 2048; D-PPO s0 Q1 reading unchanged. L1 LAUNCHED by the chain at WSL 07:47 (run_7l.sh; state /tmp/rl_7l_L1; rows rl/artifacts/v7/7l/lanes_R0_L1.log). Queued after L1: hard_battery.sh L1 ck_3072 (vs CP7 + H2H vs L0 ck_3072), Q2 row, rl/replay_ck.sh on L0 ck_3072 (transcripts -> rl/artifacts/v7/7l/replay/), then run_7d3.sh seed 1, then rebattery_c1.sh (512/1024/1536).
- 2026-09-13 15:05 (WSL 10:00): L1 DONE (rc=0, post-processed to rl/artifacts/v7/7l/L1): +0 0.86/0.87/0.88, +512 0.87/0.88/0.88, +1024 D0 0.68 [0.583,0.763] / D1 0.66 / TWIN 0.58 (44.5 turns, under/over 594/34) = a REGRESSION vs L0's +1024 0.91 [0.838,0.952]; sampled curve block 0 (vs ck_1024) 0.00-0.09 (stalls vs losses not separable: per-job RL|summary lines were discarded - FIXED now in both lane scripts: jobs.log), block 1 (vs ck_2048) 0.34 -> 0.78. RUNNING: rl/hard_battery.sh L1 ck_3072 vs CP7 then head-to-head vs L0 ck_3072 (server 7948/7949, driver 7914; log rl/artifacts/v7/7l/hard/L1_3072.log). NEXT: Q2 row; rl/replay_ck.sh on L0 ck_3072; run_7d3.sh seed 1 + waiter; HANDOFF-V7 UPDATE + §L.
- 2026-09-13 16:15 (WSL 11:10): GOTCHA (cost an hour): a 'setsid nohup ... &' launched inside a wsl -e bash -lc call no longer survives the call's end on this box (it did earlier tonight) - the 10:05 hard-battery launch (chained behind && ) and an 11:07 relaunch both died silently; and a pgrep pattern with '\|' inside the wsl string is a literal pipe for pgrep (ERE) - every 'no process' reading made that way was false. RULE NOW: long jobs via the Bash tool's run_in_background on a FOREGROUND wsl command (it keeps the wsl session alive), verify with pgrep -fa 'a|b' (plain |). L1 ck_3072 vs CP7 = 67/100 = 0.670 [0.573,0.754] (losses 27, stalls 6, 39 turns) vs L0's 0.80 [0.711,0.867]. Head-to-head L1 vs L0 RUNNING (background Bash task; log rl/artifacts/v7/7l/hard/L1_3072_h2h.log).
- 2026-09-13 16:30 (WSL 11:25): Q2 ROW WRITTEN ('7d Q2 - L1 vs L0'): head-to-head L1 0.76 [0.668,0.833] (2 losses, 22 stalls); vs CP7 L1 0.67 [0.573,0.754] vs L0 0.80 [0.711,0.867] (overlap, L1 below); heuristic family L1 +1024 0.68 vs L0 0.91 (clear regression). Verdict: NOT 'league helps'; self-play overfit signature. NEXT: replay_ck.sh L0 ck_3072 -> rl/artifacts/v7/7l/replay/; D-PPO seed 1 (run_in_background); HANDOFF UPDATE + §L.
- 2026-09-13 16:40 (WSL 11:30): TRANSCRIPTS of L0 ck_3072 written (rl/replay_ck.sh; rl.debug must be a JOB flag, the driver clears rl.* between jobs): rl/artifacts/v7/7l/replay/L0_3072_vs_heuristic_s6001.txt (won, 29 turns, 150 lines) and L0_3072_vs_cp7_s6002.txt (won, 25 turns, 165 lines); 7947/7913 stopped. NEXT: D-PPO seed 1 via run_in_background (rl/run_7d3.sh, s0 skipped), then rl/rebattery_c1.sh beside it (2 servers + 2 JVMs), then the HANDOFF UPDATE + §L.
- 2026-09-13 16:55 (WSL 11:50): D-PPO seed 1 RUNNING (relaunched detached under a WSL keepalive session - Bash background task bwnyju7xi, 3 h; RE-ARM it before it expires or the VM stops with the lane: wsl -e bash -lc 'while true; do sleep 300; done' as a background task); lane /tmp/rl_7d3_s1, driver 7910 / server 7941, run_7d3.sh post-processes s1 itself into rl/artifacts/v7/7d3/s1. rl/rebattery_c1.sh RUNNING beside it (512/1024/1536 of both seeds; server 7947 / driver 7913; rows rl/artifacts/v7/7c1/rebattery/s<seed>/rows.txt). HANDOFF-V7.md UPDATE + §L written. Owed: two-seed Q1 row when s1 lands (+ fill_na if needed); the fixed-protocol 512/1024/1536 rows into the C1 paragraph.
- 2026-09-13 19:00 (WSL 13:55): D-PPO seed 1 DONE (2048 D0 0.81 [0.722,0.875]); TWO-SEED Q1 ROW WRITTEN ('7d Q1 - D-PPO two seeds vs C1 two seeds'): pooled 2048 D0 149/200 = 0.745 [0.680,0.800] vs C1 fixed-protocol 172/200 = 0.860 [0.805,0.901] -> 'IL hurts' by 0.005 of interval (marginal; D1/TWIN clear with more room); 512 0.745 vs 0.690 overlapping; first-8 163+165/512; s1's ck_2048 argmax census identical to bc.pt; over-attacking (46/283, 138/371) vs C1's under-attacking. Idle JVM 7910 stopped. STILL RUNNING: rl/rebattery_c1.sh (s1 1024, then 1536 x2; server 7947 / driver 7913) under the keepalive - the C1 correction paragraph gets those rows at REBAT|done, then the keepalive is stopped.
- 2026-09-13 19:20 (WSL 14:15): NIGHT CLOSED. rl/rebattery_c1.sh done (every fixed-protocol point inside its plain-argmax interval; appended to the C1 correction paragraph). Nothing running in WSL (pgrep policy_server|RLDriverServer|rung0_lane|rebattery empty); keepalive stopped. Owed (see HANDOFF-V7 §L.1): card-level BC, joint-site labels, a PFSP league, C1 seed 2, the 5d update lever, the §2c contract commit.
