...
class League:
    def __init__(self, a):
        ...
        if os.path.exists(self.state_path):
            ...
            say("L10", "resume", f"n={self.st['n']}", f"sched_i={self.st['sched_i']}", f"stage={self.st['stage']}",
            ...
            say("L10", "start", f"art={self.art}", f"learners={','.join(a.learners)}",
    ...
    def resolve(self, name, bucket, rng):
        ...
        if bucket == "cross":
            others = [m for m in MAIN_DECK if m != name and m in self.st["learners"]
                      and self.st["learners"][m]["graduated"] and self.st["learners"][m]["latest"]]
    ...
    def mint_init(self, name, L):
        ...
        if not os.path.exists(path):
            ...
            say("L10", "init", name, f"gen={L['gen']}", f"seed={L['init_seed']}", line[-1] if line else out.stderr[-300:])
        return path
    ...
    def run_block(self, name):
        ...
        if pend and pend["learner"] == name and pend["gen"] == L["gen"]:
            spec = pend
            say("L10", "rerun", name, f"budget={spec['budget']}", f"opp={spec['opp']}")
        else:
            if name in EXPL_OF and not self.st["learners"][EXPL_OF[name]]["latest"]:
                say("L10", "skip", name, "main has no snapshot yet")
                return True
            ...
            spec = {"learner": name, "gen": L["gen"], "bucket": bucket, "draw": round(draw, 4), "opp": opp,
                    ...
                    "probe_due": bool(is_main and (L["blocks"] + 1) % a.probe_every == 0),
        ...
        for attempt in (1, 2):
            ...
            if rc == 0 and ta >= spec["budget"] - a.chunk:
                break
            say("L10", "error", name, f"attempt={attempt}", f"rc={rc}", f"trained={ta}", f"lane_log={L['out']}/lane.log")
        ...
        if spec["first"] and is_main:
            ...
            if v0 is not None:
                ...
                say("L10", "probe0", name, f"wr={float(v0):.3f} [{lo0:.3f},{hi0:.3f}]", f"games={a.probe_games}")
        ...
        say("L10", "block", f"n={self.st['n']}", f"h={self.hour():.2f}", f"stage={self.st['stage']}", f"learner={name}",
        ...
        if is_main:
            ...
            if pw != "" and pw >= 0.65 and not L["graduated"]:
                ...
                say("L10", "grad", name, f"trained={ta}", f"probe={pw:.3f} [{plo:.3f},{phi:.3f}]", f"h={self.hour():.2f}")
        else:
            ...
            if reset:
                ...
                say("L10", "reset", name, f"gen={L['gen']}->{L['gen'] + 1}", f"why={reset}", f"wr={wr:.3f}",
    ...
    def check_stage2(self):
        ...
        if len(grads) >= 2 or forced:
            ...
            say("L10", "stage2", f"n={self.st['n']}", f"h={self.hour():.2f}", f"graduated={','.join(grads) or 'none'}",
    ...
    def run(self):
        ...
        while True:
            ...
            if name not in self.st["learners"]:
                self.st["sched_i"] += 1
                continue
        ...
        say("L10", "done", f"reason={reason}", f"blocks={self.st['n']}", f"h={self.hour():.2f}",
...
def main():
    ...
    ap.add_argument("--learners", default="M_W,M_D,M_L,X_W,X_D,X_L")
    ap.add_argument("--schedule", default=",".join(SCHEDULE))
    ap.add_argument("--block", type=int, default=256)
    ap.add_argument("--chunk", type=int, default=64)
    ap.add_argument("--probe-every", type=int, default=4)
    ap.add_argument("--probe-games", type=int, default=50)
    ...
    a.art = a.art or (f"{LB}/rl/artifacts/v7/10" if a.tag == "main" else f"{LB}/rl/artifacts/v7/10/{a.tag}")
    a.out_root = a.out_root or ("/tmp/rl_10_" if a.tag == "main" else f"/tmp/rl_10{a.tag}_")
