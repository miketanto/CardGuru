...
- 2026-09-16 09:22Z WSL (SECOND SERIES COMPLETE - the clauses become answerable and they PASS): five clones rebuilt at --logit-bound 0, three evaluations run, all against the four clauses fixed in b512184 before any number existed. Clone table: every frozen arm gains 0.09-0.14 top-1 and 0.14-0.19 class (W0Base 0.684->0.776, W1Fly 0.632->0.775, W1Fst 0.661->0.764, W1Fst joint 0.678->0.794), while the CONTROL - the one first-series arm that never froze, W1Fly joint - is unchanged (0.791->0.780). Rescue, not uniform lift. RESULTS on priority: aspect-is-learnable MET on both rungs (1a 0.910 of ceiling vs first-series 0.734; 1b 0.901 vs 0.766, bar 0.80) - the clause whose failure VOIDED both transfer readings, so they are now answerable; shared-transfers MET with content (ratios 1.000 and 1.035, and both arms far above the shared trivial predictor on exact AND class, unlike the first series); aspect-does-not-transfer NOT MET on either (zero-shot aspect is HIGHER than its own shared figure); joint-helps neither materially - contradicting the first series rung-1a impression that JOINT carried the result, which was an artefact of everything else being frozen. Degeneracy gone: arms now differ from each other and from the trivial rule, and rung 0's anchor moves from 0.6791 (exactly trivial) to 0.7642. LADDER-LEVEL: the shared game transfers across every rung tested and the aspect does not fail to transfer - but with joint attack/block uncloned (coverage 0.56-0.65 on all four decks) that means only that the new card's PRESENCE does not disturb priority and target choice, and says nothing about legal blocks, trade math or race arithmetic. First series stands as the reading of record; series never pooled.
- 2026-09-16 09:23Z WSL (pooled AUX probe - the last owed A2/A3 item - and it REVERSES the narrow result): six decks, 54 distinct cards, 32 held-out identities, 18 informative off-wire columns, 10,811 unseen-card entities. AUX 0.9804 now ABOVE BC 0.9632 and at the frozen-embedding ceiling 0.9789; on cards unseen by the embedding itself AUX 0.9956 vs BC 0.9488, floor 0.7398. The A3 row's 'the remedy made the probe worse' (0.8590 -> 0.8232) holds only on the thin 13a-only population and does not survive pooling - corrected in the open, with the A3 readings of record unchanged. CAVEAT ON THE REVERSAL, stated with it: the majority-class baseline on this pooled population is 0.9099 (0.9356 on unseen-to-embedding), so BC, AUX and the ceiling all sit in the top nine points of the scale and the BC-to-AUX gap is 0.017 of range that imbalance already covers; one column is pathological at 0.05. Neither population is clean - the narrow one is thin, the wide one is saturated - and the auxiliary loss question is NOT settled by this probe in either direction. What survives both: the card-blind floor stays clearly separated everywhere, so identity information is present in the encoder and absent without the card table. OWED AND NOT BUILT: the Phase 8 / Phase 10 / 13c re-reading against per-deck CombatMath coverage, and engine instrumentation naming WHICH declarations the candidate list misses.

## Context checkpoint (2026-09-16 09:25Z)

STATE:      Nothing running. Ladder exited cleanly on its STOP file (rl/artifacts/v7/15/a4/STOP present; W1Lif never started). chain_15.sh finished. No drivers, no policy servers, no GPU jobs. Box idle, ~11 GB free.
DONE:       A0 PARTIAL (V7-VALIDATION 15/A0); A1 DATA-LIMITED, both halves (15/A1); A2 DISCARDS by the bar, with saturation correction and central card-blind finding (15/A2 + addendum); A3 remedy works, two addenda including the pooled reversal (15/A3); A4L first series rungs 0/1a/1b/1c all recorded, 1b and 1c marked NOT INTERPRETABLE (15/A4L rows); A4L second series at --logit-bound 0 for rungs 0/1a/1b with clauses re-scored (15/A4L SECOND SERIES); CombatMath coverage finding (15/C); logit-bound saturation finding (15/D); phase verdict (15 - phase verdict).
OPEN:       (1) Phase 8 / Phase 10 / 13c re-reading against per-deck CombatMath coverage - not started. (2) Engine instrumentation naming WHICH declarations the candidate list misses - needs new plumbing, deliberately not built. (3) A4L rungs 1d, 2, 3, 4, 5 and the black branch - recordings not made; rung 1c has a first-series row only. (4) Whether the logit bound should change project-wide - the diagnostic says it costs 0.09-0.14 top-1 in the A4L regime and nothing in healthy ones; no recommendation made.
NEXT:       Decide on (4) before any further BC or RL work, since every clone in the project trains under B=5. The cheap next measurement is a --logit-bound 0 re-clone of bc.pt itself on the 13a recordings, compared against the published 13b row - one GPU job, about 25 minutes.
GOTCHAS:    A watcher's argv is part of the system - a pgrep-based wait loop matched my own watcher and idled the box 20 minutes; wait on FILES. gate_15rec.sh is skip-if-exists, so a deck gated while partially recorded keeps stale numbers (this corrupted rung 0's first gate). p15_a4.py scores all four kinds while clones train on prio,target only, so its all-kinds column is below trivial and unusable. stop_15rec.sh cannot match run_15recall.sh, so STOPALL must be touched first. 13c's .pt checkpoints were gitignored and are gone - its saturation question is unanswerable from its own artifacts.
COMMITS:    All pushed to v7/lane-d through 2cfd8ce. Nothing uncommitted.
- 2026-09-16 09:26Z WSL (checkpoint CORRECTION - 'Nothing running' was imprecise, and the reason is instructive): a process check after the verdict returned 2 matches, which contradicted the handoff. Neither was work. One was my own polling shell matching its own argv; the other was a WATCHER shell launched 5h14m earlier, still looping on rl/artifacts/v7/15/a2/a2_probe3.json - a filename that never existed, because A2's probe 3 wrote a2_offdeck.json. A bracketed re-check showed trainers 0, servers 0, and 'drivers 1' that was again my own shell: pgrep -af 'RLDriverServe[r]' returns nothing and no engine port is listening. So the box WAS idle; the checks were lying, for the third time tonight, through argv self-matching. Added rl/stop_watchers15.sh (its own name matches none of its patterns) to kill leftover watcher loops specifically, and ran it. STANDING LESSON for the next session, now recorded three times over: a watcher's command line is part of the system state. Wait on FILES, not on process names; when a health check must use pgrep, bracket the pattern AND confirm a positive with ps or a listening port before believing it. A stale watcher is not merely litter - it jammed chain_15.sh for 20 minutes and it made a clean box look occupied.
