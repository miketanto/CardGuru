...
UPDATE (2026-09-13 16:50, next-session clock; WSL 11:45): **OVERNIGHT-7D executed** (runbook `rl/OVERNIGHT-7D.md`, STATE lines are the step-by-step record; every row in `rl/V7-VALIDATION.md`). Findings: (1) **C1 two seeds**: pooled 2048 D0 0.850 [0.794, 0.893] plain argmax / **0.860 [0.805, 0.901]** under the fixed protocol; third land fails on seed 1 (0.38); seed 1 dipped to 0.49 at 1024. (2) **Correction**: `--argmax-classes` was inert on the v7 serving path (act_v7 eval = plain argmax); fixed + tested (14/14, v7_check 15/15); the paired re-battery of both seeds' ck_2048 moves nothing (0–2 games). (3) **7d piece (1b)**: CP7 teacher seat (`CP7TeacherPlayer`, `rl.agent=cp7`, per-consult `y`, hello `teacher`), 20,735 labelled priority consults / 1,400 games, CP7 never passes a window it can act in, LAND 41 % / SPELL 59 %, CP7 0.684 [0.659, 0.708]; priority-only (joint sites not lifted). (4) **BC** (`rl/v7_bc.py`): held-out CE 0.713 / top-1 0.606 / type 0.887 vs the copy ceiling 0.280 / 0.833 — learned CP7's type, not its card; bc.pt never passes and attacks/blocks everything; BC battery D0 0.58 [0.482, 0.672]. (5) **Q1, one seed**: D-PPO s0 2048 D0 0.68 [0.583, 0.763] clear below C1 → "IL hurts" (dead stretch gone, 163/256, but a rigid BC-shaped policy PPO never left); seed 1 RUNNING. (6) **Q2, one seed each**: L0 (+1024 vs heuristic) 0.91 [0.838, 0.952] D0, 0.80 vs CP7; L1 (league vs own ancestors) 0.68 D0 (clear regression), 0.67 vs CP7 (below, overlapping), beats L0 head-to-head 0.76 [0.668, 0.833] with 22 stalls → NOT "league helps"; self-play overfit signature. (7) Lane fixes: `start_server` fork race (stale "policy server" line → probes refused → NA rows; `: > server.log` before launch, both lane scripts), `jobs.log` (per-job RL|summary lines kept), `R0_INIT` / `R0_BATTERY_START` hooks, own-port driver stop; `rl/fill_na_batteries.sh`, `rl/battery_ck.sh`, `rl/hard_battery.sh`, `rl/replay_ck.sh` (transcripts of L0 ck_3072 in `rl/artifacts/v7/7l/replay/`). Machine rules learned: 11 GB WSL box — a JVM is 3.4 GB RSS, a server 1.7 GB: ≤ 2 servers + 2 JVMs at once; the WSL VM shuts down when no `wsl` session is open, so detached launches need a keepalive session (a background `wsl -e bash -lc "while true; do sleep 300; done"` from the Bash tool); `pgrep` patterns inside a `wsl` string use plain `|`.

## L. The handoff prompt (2026-09-13 16:50; v7/lane-d = HEAD). Supersedes §K.

Read CLAUDE.md, then `rl/OVERNIGHT-7D.md` (the runbook + its STATE lines = the night's record), then `rl/V7-VALIDATION.md` from "7d overnight — correction in the open" to the end (C1 two seeds, the correction, the recording, BC, D-PPO seed 0, L0, the Q2 amendment, the C1 fixed-protocol paragraph, Q2), then §K's environment block above (still valid) plus the machine rules in the UPDATE line above.

STATE at handoff: **D-PPO seed 1 RUNNING** (`rl/run_7d3.sh` detached under the keepalive; lane `/tmp/rl_7d3_s1/`, driver 7910 / server 7941; rows mirrored nowhere — read `/tmp/rl_7d3_s1/lane.log`; the runner post-processes into `rl/artifacts/v7/7d3/s1/` itself; ~2.5 h). **C1 re-batteries 512/1024/1536 RUNNING** beside it (`rl/rebattery_c1.sh`, server 7947 / driver 7913, rows in `rl/artifacts/v7/7c1/rebattery/s<seed>/rows.txt`). Nothing else may run beside those two (memory).

NEXT, in order:
1. When `7D3|seed=1|rc=` appears in `rl/artifacts/v7/7d3/run_7d3.log`: if any `D0=NA` row, `bash rl/fill_na_batteries.sh /tmp/rl_7d3_s1 rl/artifacts/v7/7d3/s1_rebat 7948 7914`; then `python3 rl/summ_7c1.py rl/artifacts/v7/7d3/s0 rl/artifacts/v7/7d3/s1` and write the **two-seed Q1 row** against the pre-registration (pooled 200 games vs C1's fixed-protocol pooled 0.860 [0.805, 0.901]).
2. The C1 fixed-protocol re-battery paragraph gets the 512/1024/1536 rows when `REBAT|done` lands (compare to the plain-argmax rows; B7's prediction lives in the early checkpoints).
3. Owed and pre-stated follow-ups, not run: a card-level BC (lr 3e-4, no early stop, or a per-class loss) before concluding on IL; joint-site labels (lift `jointAttacks`/`jointBlocks`); a league with PFSP/sampled opponents rather than two argmax ancestors; C1 seed 2; the 5d update-cost lever (`update_profile.py`); B8/B6 ablations; the §2c width-21 contract commit.
4. Every launch: `setsid nohup bash rl/<script>.sh > /mnt/c/.../log 2>&1 < /dev/null &` inside ONE wsl call, with a keepalive wsl session alive (see the UPDATE line), verified with `pgrep -fa 'a|b'`; kill only via script files (`rl/stop_*.sh`); ≤ 2 servers + 2 JVMs.

UPDATE (2026-09-13 19:05, next-session clock; WSL 14:00): **Q1 two seeds DONE** — row "7d Q1 — D-PPO two seeds vs C1 two seeds": D-PPO pooled 2048 D0 149/200 = **0.745 [0.680, 0.800]** vs C1 fixed-protocol 172/200 = **0.860 [0.805, 0.901]** → "IL hurts" at n=200 by 0.005 of interval (marginal; D1 0.755 vs 0.850 and TWIN 0.745 vs 0.885 clear with more room; per seed s0 0.68 / s1 0.81 vs C1 0.86 / 0.86); no faster start at 512 (0.745 vs 0.690, overlapping); BC removes the dead stretch on both seeds (328/512 first-8 sampled wins vs C1's 153/512); the clone's argmax census on seed 1 is identical to bc.pt at 2048; both D-PPO seeds over-attack (under/over 46/283, 138/371) where C1's under-attack. §L below is refreshed: the only thing still running at this UPDATE is `rl/rebattery_c1.sh` (C1 512/1024/1536 under the fixed protocol; so far s0 512 0.78, s0 1024 0.79, s1 512 0.60 — the plain-argmax rows were 0.77 / 0.81 / 0.61: nothing moves), whose rows go into the C1 correction paragraph at `REBAT|done`.

### L.1 refresh (2026-09-13 19:05): state and the next questions

STATE: no lane runs. `rl/rebattery_c1.sh` may still be running (check `pgrep -fa 'rebattery_c1|battery_ck'` — plain `|`); its rows are in `rl/artifacts/v7/7c1/rebattery/s<seed>/rows.txt`; when `REBAT|done` is in `rebattery.log`, append the 512/1024/1536 rows to the "C1 — correction in the open: ... FIXED argmax-classes protocol" paragraph (B7's prediction was "up on LAND-copy windows" in early checkpoints; so far it is 0–2 games at every point). A WSL keepalive session may be alive (a background `wsl -e bash -lc "while true; do sleep 300; done"`); stop it (TaskStop) once nothing runs.

The night's answers, in one line each: Q1 — type-level BC from CP7 (priority-only) then PPO with C1's recipe is worse than PPO from scratch at 2,048 episodes on W0Base (0.745 vs 0.860, two seeds pooled, marginal), though it removes the dead stretch; Q2 — a two-ancestor argmax league from C1 s0 ck_2048 beats its sibling head-to-head (0.76, 22 stalls) but regresses against everything else (D0 0.68 vs 0.91, CP7 0.67 vs 0.80), one seed each.

NEXT (pre-stated, not run): (1) a card-level BC before concluding on IL: `rl/v7_bc.py --lr 3e-4 --patience 6` (or a per-class loss so k-copy labels do not cap the fit) and read its held-out CE against the 0.280 ceiling; only if it clears the type-level 0.71 is a second D-PPO worth its GPU. (2) Joint-site labels: lift `RLPlayer.jointAttacks/jointBlocks` to package-static builders so `CP7TeacherPlayer.selectAttackers/selectBlockers` can label CP7's declared sets (the runbook's A2 plan) — the over-attacking signature says the unlabelled combat head is where the clone went wrong. (3) A league with sampled opponents (PFSP over the C1 checkpoints + the heuristic) instead of two argmax ancestors, two seeds, with `jobs.log` so stalls are counted. (4) C1 seed 2 (`rl/run_7c1.sh` skips 0/1) for the three-seed level the pre-registration asked for. (5) The 5d update-cost lever (`rl/update_profile.py profile --arch v7 --device cuda --buf rl/artifacts/v7/5d/v7f/rl_buf.pt`) and the §2c width-21 contract commit — engine idle now.

UPDATE (2026-09-13 20:45, next-session clock; WSL 19:35): **PHASE 8 executed** (runbook `rl/PHASE8-DECKS.md`, its STATE lines are the step record; every row in `rl/V7-VALIDATION.md` from "Phase 8 pre-registration" to "8c — not run"). Phase A: the Dimir census set (`7c_BenchDimir_{p1,p99,sf}`, 903 consults), `v7_land_census.py --type SPELL`, the league lane's `R0_OPP_SPEC` (heuristic / cp7 / rl per block, opponent deck per entry), `rl/battery_xdeck.sh` (four-row cross-deck suite), lane knobs `R0_ROWS` / `R0_ROWS_FINAL` / `R0_EVAL_G_INTERIM`, `hard_battery.sh SKIP_CP7`. Two user amendments before any result: fewer rows, then the two-tier budget (50-game development probes, 100-game final levels, suites read pooled over 200). **Q3 (Dimir)**: the C1 recipe AS-IS collapses on BenchDimir to a land-and-pass rail — D0 0/100 at 512, argmax-SPELL 0/257, P(SPELL) ≤ 0.01, no creature in 100 games; stopped at 512 by decision, seed 1 not run; the four "not collapsed" clauses hold while the policy is collapsed on the SPELL axis (a clause-set limitation; the SPELL census is the Dimir counter now). The one-flag mechanism test **8a-b = C1 + `--adv-norm batch`** escapes the rail (P(SPELL) 0.88 at ≥ 3 lands, third land kept 1.00) and reaches **D0 0.41 [0.319, 0.508] / D1 0.29 at 512** (one seed; flat sampled curve .33 → .34; attacks with everything, never blocks); v6 Dimir's 1024 point .555 [.486, .622] overlaps at the edge. Correction in the open: `--ent-coef` already defaults to 0.01, so the first follow-up pre-registered was void. **Q4 (diverse league)**: L2 = L0 ck_3072 + 1,024 on W0Base vs heuristic:Dimir / CP7:Burn / 8a-b-Dimir / heuristic:Burn per 256-block: the pooled 4-row suites of L2 ck_4096 and L0 ck_3072 are IDENTICAL, 153/200 = 0.765 [0.702, 0.818] each (composition moved: −4 W0Base, −3 Dimir, +6 Burn, CP7 stalls 12 → 0); home D0 0.73 [0.636, 0.807] clear below L0's 0.91 (regression); head-to-head L2 0.32 [0.237, 0.417] with 20 stalls; per-block sampled 0.70 / 0.25 / 0.73 / 0.62, 1 stall in 1,024 training games. Mechanism: racing (23.9 vs 32.4 turns, over-attacking), not L1's stalling — the league mechanism has hurt the home level twice in two ways. 8c not run (reasons in its section). Machine lessons: a session-independent WSL keepalive is `Start-Process cmd -ArgumentList '/c start "keepalive" /min wsl.exe -e bash -lc "sleep 18000"' -WindowStyle Hidden` from PowerShell (a plain hidden `Start-Process wsl.exe` exits at once); Dimir episodes are NOT slower than W0Base (0.5–0.8 games/s when the policy plays; 0.15–0.2 when it rails and gets consulted 150 times per game); `update_s` ~18 ms per stored consult on this build.

## M. The handoff prompt (2026-09-13 20:45; v7/lane-d = HEAD). Supersedes §L.

Read CLAUDE.md, then `rl/PHASE8-DECKS.md` (runbook + STATE lines = the record of Phase 8, including the two amendments and the machine lessons), then `rl/V7-VALIDATION.md` from "Phase 8 pre-registration" to the end (the 8a interim + correction, the 8a stopped row, the 8a-b pre-registration and row, the 8b / Q4 row, "8c — not run"), then §K's environment block above (still valid) and §L.1.

STATE at handoff: **nothing runs**. No lane, no policy server, no driver JVM (7910–7914 stopped), no keepalive. Checkpoints on disk (not committed): `rl/artifacts/v7/8a/s0/ck_512.pt` (the railed policy), `rl/artifacts/v7/8ab/s0/ck_512.pt` (the 0.41 Dimir policy, cdim 94), `rl/artifacts/v7/8b/L2/ck_4096.pt`; lane state `/tmp/rl_8a_s0`, `/tmp/rl_8ab_s0`, `/tmp/rl_8b_L2` (WSL-local, resumable).

The two answers, one line each: **Q3** — the C1 recipe as-is does not train on BenchDimir (a never-cast rail from the first all-lost batch under `--adv-norm auto`); with batch-centred advantages it plays Dimir at 0.41 [0.319, 0.508] after 512 episodes, one seed, flat curve. **Q4** — a diverse cross-deck league from L0 ck_3072 leaves the pooled cross-deck suite exactly where L0 was (0.765 vs 0.765, n=200) while regressing the home level (0.73 vs 0.91) and losing the head-to-head (0.32, 20 stalls); it teaches racing where the ancestors league taught stalling.

OWED, in priority order (pre-stated, none run):
1. **8a-b second seed and its 1024 point** (`SEED=1 bash rl/run_8ab.sh`; a 1024 run needs `rung0_lane.sh BenchDimir BenchDimir 1024` with the same flags) — the Dimir level is one seed at 512 and the pre-registered v6 comparison is at 1024; read the third land (B2's cost) at every ck.
2. **8a-c = C1 + `--target-kl 0.1` on Dimir** (`TKL=0.1 ADV=auto TAG=8ac bash rl/run_8ab.sh`): does a looser budget alone escape the rail, or is centring the only lever.
3. **A PFSP league** — opponent sampling by the trainee's loss rate against each member (weighted, not a fixed rotation) and a refreshed pool (the trainee's own recent checkpoints + heuristic + CP7 + the Dimir policy), two seeds, `jobs.log` per member; the two fixed leagues both regressed the home level.
4. **The update-cost profile** (`rl/update_profile.py profile --arch v7 --device cuda --buf rl/artifacts/v7/5d/v7f/rl_buf.pt`) — the 5d lever; Phase 8 measured ~18 ms per stored consult on this build, 5d recorded 58.
5. **C1 seed 2** (`rl/run_7c1.sh` skips 0/1) for the three-seed W0Base level the C1 pre-registration asked for.
6. **Joint-site teacher labels** (lift `RLPlayer.jointAttacks/jointBlocks` so `CP7TeacherPlayer` labels combat) — the over-attacking clone of Q1 and the never-blocking 8a-b policy both point at the unlabelled combat head.
7. The "not collapsed" clause set needs a per-deck action axis (SPELL on Dimir); write it into the next pre-registration rather than patching the old rows.

Every launch: `setsid nohup bash rl/<script>.sh > /mnt/c/.../log 2>&1 < /dev/null &` inside ONE wsl call, with the session-independent keepalive above started first, verified with `pgrep -fa 'a|b'` a minute later; kill only via script files (`rl/stop_*.sh`); ≤ 2 policy servers + 2 driver JVMs; the lanes stop only their own driver port, so stop idle JVMs (`bash rl/driver_server.sh stop <port>`) before a side battery.

UPDATE (2026-09-14, Phase 9 closed; v7/lane-d = HEAD): **PHASE9-PROBES executed** (runbook `rl/PHASE9-PROBES.md`, STATE lines are the record; sections "9 / P1", "9 / P4", "9 / P2", "9 / P3" and "Phase 9 verdict" in `rl/V7-VALIDATION.md`; scripts `rl/probes/{cardswap,token_probe,target_probe,instant_speed,record_proxy}.py`, `run_p{1,2,3,4}.sh`, `record_p9.sh`; artifacts `rl/artifacts/v7/9/` without the .pt / .jsonl). **P1 (deciding): the candidate scorer is identity-blind by mechanism** — text / P/T / cost / type swaps of a SPELL candidate's card move its probability by ≤ 0.00014 in every subject (init, C1s0, L0, BC, DIM), 0 strict argmax flips in 11,839 swaps, 219/219 different-card same-row SPELL candidates tied to < 1e-4; cause: `StateGraphEncoder` zero-initialises the per-edge attention bias rows and the attention/FFN output projections, and after training the `refers_to` bias is ≤ 0.0027 (att.out / ffn.out mean |W| ≤ 0.0005 vs 0.031 for the input projections) — a candidate token attends to the entities with the same weights as every other candidate and never sees *its* card. The pre-registered text-vs-P/T ratio reading is void (the P/T control is at floor) — corrected in the open. P4: keywords and P/T decodable at 1.000 from the raw row (the wire carries the 18 keyword bits) and every encoder — preserved, and irrelevant. P2: removal is size-aware through the TARGET afterstate row (DIM as recorded +0.162 [0.100, 0.227] over chance; init already +0.225; C1s0/L0 target their own creatures), text not testable (n = 6). P3: DIM 0.280 at instant speed vs CP7 0.095, entirely the forced counterspell casts; excluding them 0.077 [0.047, 0.123] vs 0.044 [0.020, 0.093], every flash creature at sorcery speed; the heuristic's timing is not readable from the wire. Nothing runs; driver 7911 and the 7947 server stopped (`record_p9.sh stop`).

## N. The handoff prompt (2026-09-14; v7/lane-d = HEAD). Supersedes §M.

Read CLAUDE.md, then `rl/PHASE9-PROBES.md` (runbook + STATE lines), then `rl/V7-VALIDATION.md` from "9 / P1" to "Phase 9 verdict", then §K's environment block above (still valid) and §M's OWED list (still owed, now reordered below).

STATE at handoff: **nothing runs**. Checkpoints as in §M plus `rl/artifacts/v7/9/init.pt` (a copy of `/tmp/rl_7c1_s0/init.pt`, the P10INIT seed-10 net). Fresh recordings on disk, not committed: `rl/artifacts/v7/9/rec_{cp7,dim}_dimir{,_b}.jsonl` (CP7 and DIM ck_512 on BenchDimir vs the heuristic, 32 games each, with the seat's choices).

The one-line answer to Phase 9: **the v7 scorer never reads which card a candidate is** — the `refers_to` attention path from a candidate token to its referent is closed in every trained checkpoint because its bias (and the attention/FFN output projections) start at exactly zero and the lane's lr 3e-5 × ~100 updates cannot move them (≤ 0.0027). Everything the policies do is a function of the candidate's own afterstate row and the game token; "which creature to cast" among same-cost cards is an exact tie broken by candidate order. This is the mechanism behind every type-level policy in Phases 7–8 and behind BC's "type, not card" (its scorer could not see the card it was asked to imitate).

OWED, in priority order (pre-stated, none run):
1. **Open the identity channel and re-test with P1 as the gate**: a `V7Policy` init variant with the `refers_to` / `referred_by` bias rows started at a non-zero value (e.g. +2 on every head, so a candidate attends to its referent by construction) and/or a separate, larger learning rate on `att.bias_rows` and the output projections (`--edge-lr`), keeping the zero-init exactness properties as tests that state the new init. Pre-register: P1's text / P/T / cost swaps must move Δp by ≥ 0.01 on the init variant *before* training (a construction check), then C1's recipe to 512 on W0Base and 8a-b's on Dimir, then P1 again at ck_512: "identity read" if the P/T Δp is ≥ 0.05 with a CI clear of the identity-blind floor; the tie census (219 same-row pairs) must break. Cannots: this does not promise a better level — the identity channel being open is necessary, not sufficient. **Already begun in parallel (Phase 10, `rl/PHASE10-LEAGUE.md`, commits ce38ccc / 7638ded, section "10 / A1-A2"):** `--cand-refers-pool` pools the referents' rows into the candidate token behind a flag (default OFF) and passes its construction gate on the untrained net (text swaps Δp 0.052 [0.049, 0.054], P/T 0.040, vs 0.000 with the flag off — measured with a copy of `rl/probes/cardswap.py`, `rl/p10_cardswap.py`). So item 1 reduces to: run `rl/probes/cardswap.py` (P1) on every Phase 10 checkpoint as the post-training gate, and the tie census must break.
2. The §M list, unchanged in content: 8a-b second seed + 1024; 8a-c (`--target-kl 0.1` on Dimir); PFSP league; update-cost profile; C1 seed 2; joint-site teacher labels; the per-deck "not collapsed" axis — but each of these trains a scorer that cannot see cards, so item 1 comes first unless the question is explicitly about type-level play.
...
UPDATE (2026-09-15 ~12:50Z WSL, Phase 12 closed for tonight; v7/lane-d = HEAD): **PHASE12-CP7 executed for Dimir only** (runbook `rl/PHASE12-CP7.md`, Amendments 1-5 and the STATE lines are the record; rows `12 — pre-registration, amendments and start points` through `Phase 12 verdict` in `rl/V7-VALIDATION.md`). M_D trained 5,376 episodes vs a fixed 75 % CP7 / 25 % heuristic mirror mix (M_D_06400 -> 11,776); **not graduated**. Argmax-classes CP7 levels 0.19 / 0.18 / 0.09 / 0.08 / 0.31 / 0.33 / end 0.21; **sampled** CP7 levels at the first four snapshots 0.12 / 0.11 / 0.15 / 0.22. The census readout shows the class argmax choosing PASS against an acting majority 0.15 -> 0.43 of PASS-offered consults, so by the pre-stated Amendment 4 criterion **the mid-phase argmax decline was a readout artifact** and the policy improved modestly under sampling (not clear). Card habits oscillated under a fixed opponent, so **self-play drift is not the main explanation** of Phase 11's swing; self-removal mostly gone. Code: `rl/phase12.py` (+ `rl/run_phase12.sh`, `rl/stop_p12.sh`, `rl/kill_p12.sh`), `rl/battery_p12s.sh` / `rl/run_p12s.sh` (sampled levels), `rl/p12_readout.py` (argmax-readout census on recorded consults), `rl/after_p12.sh`, `rl/run_p12t.sh`, `rl/policy_server.py --argmax-two-stage` (default off; tests 20/20, v7_check 15/15). Artifacts `rl/artifacts/v7/12/` (phase12.log, results.tsv, levels.tsv, census_lines.txt, sampled/, pool/*.pt gitignored). Box rule learned: **the lane + at most one other model-holding process** (a CP7 JVM is 4.7 GB RSS; two servers + two JVMs swapped the box).

## P. The handoff prompt (2026-09-15; v7/lane-d = HEAD). Supersedes §O for Phase 12's lines.

Read CLAUDE.md, then `rl/PHASE12-CP7.md` (runbook, Amendments 1-5, STATE), then `rl/V7-VALIDATION.md` from "12 — pre-registration, amendments and start points" to "Phase 12 verdict". Then §K's environment block and the §M/§N launch rules, with the box rule above replacing "≤ 2 servers + 2 JVMs".

STATE at handoff: **nothing runs.** Phase 12 state is resumable (`rl/artifacts/v7/12/state.json`; `python3 rl/phase12.py --mains M_L,M_W` resumes the paused mains from their measured start points). M_D snapshots every 256 episodes in `rl/artifacts/v7/12/pool/` (M_D_06656 ... M_D_11776).

The answer in one line: under a fixed CP7-heavy mix, Dimir did not reach competence against CP7 in 5,376 episodes. The argmax-classes readout misreports this policy family (PASS against an acting majority), and the sampled policy improved modestly where the argmax level fell.

OWED, in priority order (pre-stated, none run):
1. **The evaluation protocol decision for this policy family** (argmax-classes vs sampled vs two-stage), before any further level is read or any graduation is judged. Inputs: item 2 and the Phase 12 three-readout table.
2. **The two-stage readout (Amendment 5, cancelled).** `rl/run_p12t.sh` runs it as written: six snapshots x (CP7 100 + heuristic 50), `--argmax-two-stage`, level seeds, one evaluation at a time. Its launch waits for the sampled battery's `P12S|done` line; to run it alone, launch it with that line already present, or run its loop by hand. Pre-stated reading in Amendment 5.
3. **The remaining sampled points:** 10,496, 11,520 and 11,776 vs CP7, plus the 9,472-onward heuristic rows. `rl/run_p12s.sh` is resumable and skips finished rows; add the end snapshot to its list.
4. **The census readouts cancelled with the evaluations:** n=8 (8,704, an act-swing state), n=11, 15, 19, 20 (`rl/p12_readout.py`, CPU, the commands are in `rl/run_p12t.sh`). They separate readout flips from distribution moves in the oscillation.
5. **Landfall and white, paused at their start points** (Amendment 2): CP7 0.35 / 0.58 argmax; their Phase 12 readings are deferred, not dropped.
6. The coarse-terminal-reward question (the runbook's alternative to drift), and §O's owed list.
