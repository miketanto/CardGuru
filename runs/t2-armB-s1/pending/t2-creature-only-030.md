...
**Write your answer to `../answers/t2-creature-only-030.txt`** and nothing else: the JSON
...
your battlefield:
  - Bear Cub {1}{G} 2/2 — Creature — Bear
  - Rotting Fensnake {3}{B} 5/1 — Creature — Zombie Snake
  - Gloom Pangolin {2}{B} 1/5 — Creature — Nightmare Pangolin
...
OPPONENT (player B) — life 10
their battlefield:
  - Island — Basic Land — Island (TAPPED)
      text: ({T}: Add {U}.)
  - Island — Basic Land — Island (TAPPED)
      text: ({T}: Add {U}.)
  - Vizzerdrix {6}{U} 6/6 — Creature — Rabbit Beast (TAPPED)

COMPUTED (facts derived by the engine tooling — not advice):
- your full attack: 8 combat damage (opponent has no untapped creatures; nothing can block)
- opponent at 10: a full attack alone leaves them at 2
- your untapped lands: 2
- Shock ({R}): 2 damage, may target a player; castable now
- Roast ({1}{R}): 5 damage, targets creatures ONLY — cannot be aimed at a player; castable now
- total costs 3 > 2 untapped lands: you cannot cast everything
