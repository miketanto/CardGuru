...
**Write your answer to `../answers/t4-belief-019.txt`** and nothing else: the JSON
...
your battlefield:
  ...
  - Walking Corpse {1}{B} 2/2 — Creature — Zombie
...
Every candidate line failed in the rules engine with:

    AssertionError: Targets list was setup by addTarget with [B], but not used (Cast Searing Spear)

You targeted the opponent PLAYER the wrong way. To cast a damage spell at
the opponent's face, put the target on the cast action itself with
"target_player": "B" — do NOT use a separate {"do":"target"} action for a
player. A {"do":"target"} action is only for choosing among creatures the
engine prompts for (value = a creature name). Fix every candidate and answer
again — same file, same rules.
