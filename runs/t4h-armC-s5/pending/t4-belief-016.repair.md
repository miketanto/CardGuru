...
**Write your answer to `../answers/t4-belief-016.txt`** and nothing else: the JSON
...
your battlefield:
  - Orazca Frillback {2}{G} 4/2 — Creature — Dinosaur
  - Wetland Sambar {1}{U} 2/1 — Creature — Elk
  - Ogre Warrior {3}{R} 3/3 — Creature — Ogre Warrior
...
OPPONENT (player B) — life 8
their battlefield:
  - Mountain — Basic Land — Mountain
      text: ({T}: Add {R}.)
  - Coral Commando {2}{U} 3/2 — Creature — Merfolk Warrior
their hand (known):
  - Shock {R} — Instant
...
## REPAIR ROUND — your previous answer was invalid

Your line failed validation with:

    all candidates failed: invalid: bad phase 'COMBAT' (every candidate used phase 'COMBAT')

There is no phase called 'COMBAT'. The valid phase values are exactly the
ones listed in the vocabulary: for a "cast" action use "PRECOMBAT_MAIN" or
"POSTCOMBAT_MAIN". Attacks are their own action ({"do": "attack", ...} —
no phase field), and an instant you want to cast mid-combat is not
expressible with a 'COMBAT' phase — cast it in PRECOMBAT_MAIN before
attacking instead. Fix the line (every candidate if you answered with
several) and answer again — same file, same rules.
